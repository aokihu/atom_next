 ...
 * parentTaskId 指向直接上游任务，用于还原派生或续跑关系。
 * 根任务默认复用自身 id 作为 parentTaskId。
 */
...
export type RawTaskItem = {
  ...
  chainRound?: number; // 可选的内部连续会话轮次,只在FOLLOW_UP链路中使用
  parentTaskId: UUID | undefined; // 父任务ID,根任务默认与id一致,派生任务记录直接上游任务ID
  sessionId: UUID; // 会话ID
  chatId: UUID; // 会话中对话ID
  /* --- 任务状态 --- */
  state: TaskState;
 ...
 * @description
 * 内部任务必须显式提供链路信息，
 * 这样 Core 才能根据 parentTaskId / chainId 还原派生关系。
 */
export type InternalTaskItemInput = Pick<
  RawTaskItem,
  "sessionId" | "chatId" | "chainId" | "parentTaskId"
> &
  Partial<
    Pick<
      RawTaskItem,
